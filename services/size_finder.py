...
class SizeFinder:
    ...
    def create_size_patterns():
        ...
        for (unit_pattern, clean_unit) in units:
            all_digits_pattern = " ".join((all_digits_regex, unit_pattern))
            # create a single regex to match 10x faster
    ...
    def pattern_match(self, to_be_matched: str):
        for (pattern, unit) in self.patterns:
            ...
            if matches:
                matches = [m for m in matches.pop() if m]
                print(pattern, to_be_matched, matches)
                match = matches[0].strip()
                return match, unit

    def get_digits_unit_size(self, name):
        # add whitespace because regex groups have them at the end by default
        # eg. '\\d+ lt '
        name += " "
